...
const longestString = (arr) => {
  ...
  if (arr.length > 0) {
    let newResult = arr.reduce((a, b) => {
      return a.length > b.length ? a : b;
    });
    result = arr.indexOf(newResult);
  } else {
    result = -1;
  }
...
const firstLetters = (arr) => {
  // Solution code here...
  let result = arr.map((letter) => letter[0]);
  //console.log(result);
  return result;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let result = arr.map((e) => e.replace(/\D/g, ""));
  return result;
};
...
const allHappy = (arr) => {
  // Solution code here...
  let result =
    arr.filter((result) => !result.includes(":)")).length > 0 ? false : true;
  // console.log(result);
  return result;
};
